...
#importing data
countData = read.csv("/Users/pranavpalli/Desktop/summer_21/data/data_organization/DESEQ_files/wr1.csv", header = TRUE, row.names = 1)
```
...
dds_design = DESeqDataSetFromMatrix(countData = countData,
                             colData = colData,
                             design = ~ genotype)

dds = DESeq(dds)
dds_design = DESeq(dds_design)
```


```{r}
#y <- as.matrix(read.table(dds, header=TRUE, row.names = "Gene"))
#dd <- as.dist((1 – cor(y))/2)   #Pearson correlation  distance
...
```{r}
res = results(dds)
res_design = results(dds_design)
res_contrast = results(dds, contrast = c("genotype", "wild_type", "wrinkle"))
res_contrast_opp = results(dds, contrast = c("genotype", "wrinkle", "wild_type"))
```


```{r}
res = as.data.frame(res)
res_design = as.data.frame(res_design)
res_contrast = as.data.frame(res_contrast)
res_contrast_opp = as.data.frame(res_contrast_opp)
